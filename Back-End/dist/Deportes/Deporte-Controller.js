...
const em = orm.em;
async function validateHorario(horarios, deporte) {
...
export function sanitizeDeporte(req, res, next) {
...
export async function update(req, res) {
    try {
        ...
        await Promise.all(horariosAntiguos.map(async (horario) => {
...
export async function remove(req, res) {
    try {
        ...
        await Promise.all(horariosToCheck.map(async (horario) => {
            const isUsed = await em.findOne(Deporte, { horarios: horario });
            if (!isUsed) {
